...
const evidencePath = path.join(resultDirectory, "reputation-entry-phase-b-attempt-4-preflight.json");
const databasePath = path.join(resultDirectory, "reputation-entry-phase-b-attempt-4.sqlite");
const running = process.env.RUN_REPUTATION_ENTRY_PHASE_B_ATTEMPT_4_PREFLIGHT === "true";
...
describe("Phase B Attempt 4 source and provider preflight", () => {
  it.skipIf(!running)("passes before any Attempt 4 Job lifecycle exists", async () => {
    if (existsSync(evidencePath)) throw new Error("PHASE_B_ATTEMPT_4_PREFLIGHT_ALREADY_EXISTS");
    if (existsSync(databasePath)) throw new Error("PHASE_B_ATTEMPT_4_JOB_STATE_ALREADY_EXISTS");
    const historicalBefore = historicalIdentity();
    const preflight = await runReputationEntryPhaseBPreflight({
      attemptId: "PHASE_B_ATTEMPT_4",
      sources,
  ...
  it.skipIf(!existingEvidence)("replays the retained blocked preflight evidence without credentials", () => {
...
function linkedVercelProject() {
  ...
  if (!linked.projectId || !linked.orgId || linked.projectName !== "donelayer") {
    throw new Error("PHASE_B_ATTEMPT_4_LINKED_PROJECT_INVALID");
  }
  return {
