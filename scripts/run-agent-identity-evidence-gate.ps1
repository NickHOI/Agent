...
try {
  ...
  $identityPath = Join-Path $workspace "test-results\agent-identity-working-tree-identity.txt"
  ...
  $env:RUN_AGENT_IDENTITY_EVIDENCE_GATE = "true"
  $env:AGENT_IDENTITY_HEAD_COMMIT = $headCommit
  $env:AGENT_IDENTITY_WORKTREE_SHA256 = $identitySha256
  & node .\node_modules\vitest\vitest.mjs run tests/integration/agent-identity-evidence.test.ts
  exit $LASTEXITCODE
}
finally {
  Pop-Location
