...
$cloneRoot = Join-Path $testResults ("fixture-prep-attempt-4-" + [Guid]::NewGuid().ToString("N"))
$branches = @(
  @{ TaskType = "TEST_AND_FIX"; Branch = "donelayer/repair/reputation-async-retry-v3"; Source = "async-retry" },
  @{ TaskType = "BUILD_RESCUE"; Branch = "donelayer/repair/reputation-path-alias-v3"; Source = "path-alias-build" }
)
...
try {
  ...
  try {
    ...
    foreach ($item in $branches) {
      ...
      Invoke-Git @("add", "--all")
      Invoke-Git @("commit", "--message", "Add Phase B Attempt 4 $($item.TaskType) fixture")
