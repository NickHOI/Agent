...
$identityPath = Join-Path $resultDirectory "reputation-entry-phase-b-attempt-3-working-tree-identity.txt"
$attemptArtifacts = @(
  $identityPath,
...
$attempt2Artifacts = @(
  (Join-Path $resultDirectory "reputation-entry-phase-b-attempt-2-working-tree-identity.txt"),
  ...
  (Join-Path $workspace "REPUTATION_ENTRY_PHASE_B_ATTEMPT_2_BLOCKED.md")
)
...
try {
  foreach ($artifact in $attemptArtifacts) {
    if (Test-Path -LiteralPath $artifact) {
      throw "REPUTATION_ENTRY_PHASE_B_ATTEMPT_3_EVIDENCE_ALREADY_EXISTS: $([IO.Path]::GetFileName($artifact))"
    }
  }
  foreach ($artifact in $attempt2Artifacts) {
    if (-not (Test-Path -LiteralPath $artifact)) {
      throw "REPUTATION_ENTRY_PHASE_B_ATTEMPT_2_HISTORY_MISSING: $([IO.Path]::GetFileName($artifact))"
    }
  }
  $attempt2IdentityBefore = @{}
  foreach ($artifact in $attempt2Artifacts) {
    $attempt2IdentityBefore[$artifact] = Get-Sha256Hex ([IO.File]::ReadAllBytes($artifact))
  }
  ...
  $identityLines.Add("ATTEMPT PHASE_B_ATTEMPT_3")
  ...
  $env:RUN_REPUTATION_ENTRY_PHASE_B_ATTEMPT_3 = "true"
  ...
  foreach ($artifact in $attempt2Artifacts) {
    $after = Get-Sha256Hex ([IO.File]::ReadAllBytes($artifact))
    if ($after -ne $attempt2IdentityBefore[$artifact]) {
      throw "REPUTATION_ENTRY_PHASE_B_ATTEMPT_2_HISTORY_CHANGED: $([IO.Path]::GetFileName($artifact))"
    }
  }
  exit 0
}
