...
try {
  ...
  $identityPath = Join-Path $workspace "test-results\task-scoped-authority-working-tree-identity.txt"
  ...
  $env:RUN_TASK_SCOPED_AUTHORITY_REALITY_GATE = "true"
  $env:TASK_SCOPED_AUTHORITY_HEAD_COMMIT = $headCommit
  $env:TASK_SCOPED_AUTHORITY_WORKTREE_SHA256 = $identitySha256
  & node .\node_modules\vitest\vitest.mjs run tests/integration/task-scoped-authority-reality.test.ts
  exit $LASTEXITCODE
}
finally {
  Pop-Location
